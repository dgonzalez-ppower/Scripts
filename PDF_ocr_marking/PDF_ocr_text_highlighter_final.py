...
def marcar_textos_mejorado(pdf_path, textos_objetivo, output_pdf_path, dpi=300):
    ...
    for i, page in enumerate(doc):
        ...
        for idx, objetivo in enumerate(textos_objetivo):
            ...
            # Búsqueda difusa por palabra individual
            for palabra in palabras:
                if parece_igual(palabra['text'], objetivo):
                    ...
                    print(f"🟠 Coincidencia aproximada con '{palabra['text']}' ≈ '{objetivo}'")

            # Búsqueda por reconstrucción dentro de cada línea
            for linea in lineas:
                reconstrucciones = reconstruir_en_linea(objetivo, linea)
                for combinada, grupo in reconstrucciones:
                    print(f"🧩 Reconstrucción en línea: '{combinada}' ≈ '{objetivo}'")
...
output_pdf_path = os.path.join(os.path.dirname(pdf_path), f"{pdf_name}_marked_final.pdf")

print("🚀 Iniciando escaneo del PDF página por página...")
marcar_textos_mejorado(pdf_path, textos_usuario, output_pdf_path)
