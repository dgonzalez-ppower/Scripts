...
def parece_igual(cadena1, cadena2, umbral=0.65):
    ...
    if cadena1 == cadena2:
        return True
    if cadena2 in cadena1 and cadena1.startswith(cadena2):  # extra solo al final
        return False
    if cadena2 in cadena1 and cadena1.endswith(cadena2):  # extra solo al inicio
        return False
    if cadena2 in cadena1 and len(cadena1) == len(cadena2):  # casi iguales
        return True
...
def marcar_textos_purificado(pdf_path, textos_objetivo, output_pdf_path, dpi=300):
...
output_pdf_path = os.path.join(os.path.dirname(pdf_path), f"{pdf_name}_marked_cleanmatch.pdf")

print("🚀 Iniciando escaneo del PDF página por página...")
marcar_textos_purificado(pdf_path, textos_usuario, output_pdf_path)
