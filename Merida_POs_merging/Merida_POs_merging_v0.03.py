...
def main():
    ...
    for file in files:
        ...
        try:
            ...
            fill = fill_blue if fill_toggle else fill_white
            fill_toggle = not fill_toggle

            # Insert local header first
            ws_out.append([po_number, revision, supplier] + headers)
            row_pointer += 1
            
            for row in data_rows:
                ws_out.append([po_number, revision, supplier] + row)
            row_pointer += len(data_rows)

            # Luego aplicamos estilo a TODO el bloque (encabezado + datos)
            start = row_pointer - len(data_rows) - 1  # el -1 es por el encabezado recién agregado
            end = row_pointer - 1
            for r in range(start + 1, end + 2):  # +2 porque range es exclusivo
                for c in range(1, 16):
            ...
            ws_out.append([po_number, "NO DATA FOUND", "NO DATA FOUND"] + ["NO DATA FOUND"] * 12)
