...
def split_pdf_by_bookmarks(original_name, pdf_path, bookmarks):
    ...
    # Usar solo bookmarks de nivel 0 y que no sean Cover para dividir
    top_bms = [b for b in bookmarks if b["level"] == 0 and str(b["title"]).strip().lower() != "cover"]
    include_cover = len(top_bms) > 1  # Si hay más de un marcador de nivel 0, se añade Cover
    ...
    for i, bm in enumerate(top_bms):
        ...
        # Sufijo para nombrar el archivo
        ...
        # Añadir marcador raíz con el nombre del archivo
        root_title = os.path.splitext(os.path.basename(output_filename))[0]
        try:
            root_bookmark = writer.add_outline_item(root_title, 0)
        except Exception as e:
            print(f"[WARN] No se pudo crear marcador raíz '{root_title}': {e}")
            root_bookmark = None

        # Insertar todos los bookmarks dentro del rango como hijos del root
        for b in bookmarks:
            if str(b["title"]).strip().lower() == "cover":
                continue
            if start_page <= b["page"] < end_page:
                try:
                    page_in_output = b["page"] - start_page + page_offset
                    ...
                    print(f"[WARN] No se pudo añadir bookmark '{b['title']}': {e}")
