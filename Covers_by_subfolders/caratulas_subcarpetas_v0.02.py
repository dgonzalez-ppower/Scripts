...
def generate_covers_recursive(template_path, root_folder):
	folders_created = 0

	# Primero, generar carátula para la carpeta raíz
	root_folder_name = os.path.basename(root_folder)
	root_docx_output = os.path.join(root_folder, f"{root_folder_name}_COVER.docx")
	try:
		replace_placeholder_and_save(template_path, root_folder_name, root_docx_output)
		convert_docx_to_pdf(root_docx_output)
		folders_created += 1
	except Exception as e:
		print(f"⚠️ Error processing root folder: {e}")

	# Luego, generar para subcarpetas recursivamente
