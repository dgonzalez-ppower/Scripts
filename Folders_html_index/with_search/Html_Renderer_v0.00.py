  ...
  <script>
    ...
    searchBox.addEventListener('input', function () {
      ...
      results.forEach(r => {
        ...
        div.innerHTML = div.innerHTML = `<strong>${doc.filename}</strong><div class='filepath'><a href="${doc.filepath}" target="_blank">${doc.filepath}</a></div>`;
...
def generate_index_html(json_folder, output_html, source_folder):
    ...
    for file in json_folder.glob("*.json"):
        with open(file, encoding='utf-8') as f:
            content = json.load(f)
            # usamos directamente la ruta relativa del JSON
