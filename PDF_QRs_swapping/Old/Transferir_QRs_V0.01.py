...
# Configuración
DPI = 400
MARGEN_QR = 5  # en puntos PDF (~1.7 mm)
...
for page_num in range(len(scanned_doc)):
	...
	# Ampliar ligeramente el área de limpieza con un margen extra
	clean_rect = fitz.Rect(
		target_rect.x0 - MARGEN_QR,
		target_rect.y0 - MARGEN_QR,
		target_rect.x1 + MARGEN_QR,
		target_rect.y1 + MARGEN_QR
	)
	page.draw_rect(clean_rect, fill=(1, 1, 1), color=(1, 1, 1), overlay=True)

	# Insertar el QR de alta calidad encima
