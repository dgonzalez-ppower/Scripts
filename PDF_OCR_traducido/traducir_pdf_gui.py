...
def translate_lines_by_page(pages_text, batch_size=5):
    translated_pages = []
    ...
    for page_num, lines in enumerate(pages_text, start=1):
        ...
        # Traducir en bloques de N líneas
        for i in range(0, len(lines), batch_size):
            block = lines[i:i + batch_size]
            src_text = "\n".join(line.get("text", "") for line in block)

            try:
                translated_block = GoogleTranslator(source="auto", target="es").translate(src_text)
            except Exception as e:
                print(f"⚠️ Fallo al traducir bloque en página {page_num}: {e}")
                translated_block = src_text

            # Procesar resultado, línea por línea
            if not isinstance(translated_block, str):
                translated_block = str(translated_block)

            split_translations = [
                l.strip() for l in translated_block.replace("\r", "").splitlines() if l.strip()
            ]

            # Ajuste si hay más o menos líneas traducidas que originales
            if len(split_translations) < len(block):
                print(f"⚠️ Página {page_num}: traducción parcial en bloque. Recuperando faltantes…")
                split_translations += [line["text"] for line in block[len(split_translations):]]
            elif len(split_translations) > len(block):
                print(f"⚠️ Página {page_num}: traducción con líneas extra. Truncando resultado…")
                split_translations = split_translations[:len(block)]

            for original, translated in zip(block, split_translations):
                translated_lines.append({
                    "text": translated,
