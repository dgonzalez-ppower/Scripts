...
<body>
  ...
  <script>
    ...
    function wildcardToRegex(pattern, withAnchors = false) {
        // 1. Elimina espacios extra
        pattern = pattern.trim().replace(/\s+/g, '');
        // 2. Escapa todo lo que sea especial en regex menos * y ?
        ...
        if (withAnchors) {
            ...
            return new RegExp(pattern, 'gi'); // busca todas las ocurrencias
        }
    }
    ...
    searchBox.addEventListener('input', function () {
        ...
        let showResults = [];
        let useWildcard = /[*?]/.test(query);

        if (useWildcard) {
            // Wildcard: filtra manualmente como Windows Explorer
            const regexFile = wildcardToRegex(query, true);    // Para nombre de archivo
            const regexContent = wildcardToRegex(query, false); // Para buscar dentro del texto (sin anchors)
            showResults = documents.filter(doc =>
                regexFile.test(doc.filename) ||
                (doc.text_content && regexContent.test(doc.text_content))
            );
        } else {
            // Exacto/substring: usa lunr si quieres (o solo filtro simple)
            // Aquí se prioriza match exacto de nombre, luego substring, luego lunr
            showResults = documents.filter(doc => doc.filename.toLowerCase() === query.toLowerCase());
            if (showResults.length === 0)
                showResults = documents.filter(doc => doc.filename.toLowerCase().includes(query.toLowerCase()));
            if (showResults.length === 0 && typeof idx !== "undefined") {
                const lunrResults = idx.search(query);
                showResults = lunrResults.map(r => documents[r.ref]);
            }
        }
        ...
        searchCountDiv.innerHTML = `<strong>${showResults.length}</strong> document(s) found`;

        showResults.forEach(doc => {
            ...
            if (useWildcard) {
                // Coincidencia de nombre exacta tipo Windows
                const regexFile = wildcardToRegex(query, true);
                if (regexFile.test(doc.filename)) {
                    fileMatch = true;
                    hits += 1; // Puedes mostrar como "filename hit"
                }
                // Coincidencia de contenido página a página
                if (doc.page_texts) {
                    const regexContent = wildcardToRegex(query, false);
                    doc.page_texts.forEach(p => {
                        const matchCount = (p.text.match(regexContent) || []).length;
                        if (matchCount > 0) {
                            hits += matchCount;
                ...
                // Sin wildcard: substring clásico
            ...
            // Render solo si hay hit en nombre o hits > 0
            if (fileMatch || hits > 0) {
                div.innerHTML = `
