...
<body>
  ...
  <script>
    ...
    searchBox.addEventListener('input', function () {
        ...
        // Quita espacios al principio y al final, y si sólo hay espacios, sal
        ...
        if (useWildcard) {
            // Windows-style wildcard (espacio = .*)
            const regex = wildcardToRegex(query, true);
            showResults = documents.filter(doc =>
                regex.test(doc.filename) ||
                (doc.text_content && regex.test(doc.text_content))
            );
        } else {
            // Búsqueda normal (case insensitive)
        ...
        showResults.forEach(doc => {
            ...
            if (useWildcard) {
                const regexInner = wildcardToRegex(query, false); // SIN anchors
                if (doc.page_texts) {
                    doc.page_texts.forEach(p => {
                        const matchCount = (p.text.match(regexInner) || []).length;
                        if (matchCount > 0) {
                            hits += matchCount;
                ...
                // También cuenta coincidencia en filename si quieres
                if (regexInner.test(doc.filename)) hits++;
            } else {
                ...
                if (doc.filename.toLowerCase().includes(lowerQuery)) hits++;
            }

            // Renderizar solo si hay hits o el doc coincide por nombre (ya está en el filtro)
            div.innerHTML = `
