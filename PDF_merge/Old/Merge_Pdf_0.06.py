...
def merge_tree(node, writer, current_offset, parent_bookmark, pdf_reader_cache):
    ...
    def add_internal_bookmarks(outlines, parent, base_offset):
            for item in outlines:
                if isinstance(item, list):
                    ...
                    try:
                        title = item.title if hasattr(item, "title") else "Sin título"
                        page_index = reader.get_destination_page_number(item)

                        internal_bm = writer.add_outline_item(title, base_offset + page_index, parent=parent)
                        print(f"✔ Marcador interno añadido: {title} → página {base_offset + page_index}")

                        if hasattr(item, "children") and item.children:
                            add_internal_bookmarks(item.children, internal_bm, base_offset)

                    except Exception as e:
                        print(f"⚠️ Error procesando marcador interno '{title}': {e}")
    ...
    for i, pdf_path in enumerate(node['pdf_files']):
        ...
        try:
            ...
            print(f"❌ No se pudieron copiar los marcadores internos de {pdf_path}: {e}")
